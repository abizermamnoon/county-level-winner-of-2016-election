...
```{r}
#making clusters to group locations within new york city 
set.seed(7)
...
```{r eval = "FALSE"}
#calculating what number of groups to cluster by 
k<- 1:10
...
fitted_tree <- tree(fare_amount~pickup_latitude_mod+pickup_longitude_mod+ dropoff_latitude_mod+ dropoff_longitude_mod+ distance + passenger_count+pickup_month+pickup_date+pickup_time, uber_dist_cleaner)
...
uber_dist_2015 <- uber_dist_2015 %>% mutate(prediction = predict(rf, type = "response"))

uber_dist_2015 %>% summarize(accuracy = mean(prediction == fare_amount))
```
...
rf <- randomForest(fare_amount ~ pickup_latitude_mod+pickup_longitude_mod+ dropoff_latitude_mod+ dropoff_longitude_mod+ passenger_count+pickup_time+distance, data=uber_dist_2014_morning, ntree = 500)
rf
importance(rf) 

uber_dist_2014_morning <- uber_dist_2014_morning %>% mutate(prediction = round(predict(rf, type = "response"))) 
```
