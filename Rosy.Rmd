...
uber_dist <- uber_fare_mod %>% mutate(distance = dist_conversion(pickup_latitude_mod, dropoff_latitude_mod,pickup_longitude_mod,dropoff_longitude_mod))


# putting fare into different groups
uber_dist_fare <- uber_dist %>% filter(fare_amount > 0) %>% mutate(fare = ifelse(fare_amount>20,ifelse(fare_amount>40,ifelse(fare_amount>100,"too high","high"),"mid"),"low"))
