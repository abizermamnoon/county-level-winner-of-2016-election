...
```{r}
uber_dist_cleaner %>% group_by(cluster_dropoff) %>% dplyr::summarize(n = n()) #summary results of number of data points in each cluster
uber_dist_cleaner %>% group_by(cluster_pickup) %>% dplyr::summarize(n = n())
```

```{r}
#linear regression model
model <- lm(fare_amount ~ pickup_latitude_mod+pickup_longitude_mod+ dropoff_latitude_mod+ dropoff_longitude_mod+ passenger_count+pickup_month+pickup_date+pickup_time+distance, data=uber_dist_2015)
```
...
#based on the k means clustering, we can see that horizontal coordinates(longitude) is less important that vertical coordinates(latitude). Therefore, traveling within clusters has less impact on fare amount, and traveling between clusters has a bigger impact on fare amount. 
register_google(key = "AIzaSyB-C3liKGZzMpScV_lEwVlpY94-FB-NUtg", write = TRUE)

base_map_pickup <- ggmap(get_googlemap(center = c(-73.958285, 40.713000), zoom = 11))
...
base_map_dropoff <- ggmap(get_googlemap(center = c(-73.908285, 40.713000), zoom = 11))
...
base_map_path<- ggmap(get_googlemap(center = c(-73.958285, 40.723000), zoom = 11))
...
```
```{r}
#after creating a scatter plot of the fare amount and distance, facet wrapped based on time category, we can see that there is a linear relationship between those 2 variables. This scatter makes me clear that there are outliers in the data points. First of all that there are data with a distance of 0 with a non 0 fare. Perhaps this means that these are trips to and from the same location. Therefore, predicting these fares will be difficult since the data set may not have enough information. They will be considered outliers. Additionally, the horizontal lines on the scatter plot may represent fixed prices for specific locations in New York. 
ggplot(data=uber_dist_2015, aes(x=distance, y = fare_amount)) + geom_point() + facet_wrap(~time_category) + geom_smooth(method = "lm")

```

```{r}
#add count of each unique time as a new column, n
uber_dist_2015 <- uber_dist_cleaner %>% filter(pickup_year == "2015") 

uber_dist_2015 <- uber_dist_2015 %>% mutate(fare_amount_round = round(fare_amount, 0))

uber_dist_2015 %>% mutate(prediction = round(predict(rf, type = "response"),0)) %>% summarize(accuracy = mean(prediction == fare_amount_round))


time_group <- uber_dist_2015 %>% group_by(pickup_time) %>% dplyr::summarize(n = n())
left_join(uber_dist_2015, time_group, by = c("pickup_time"="pickup_time" ))


uber_dist_2015 <- left_join(time_group, uber_dist_2015, by = "pickup_time")
```

```{r}

uber_dist_2015 <- uber_dist_2015 %>% mutate(prediction = round(predict(rf, type = "response"))) 
...
rf <- randomForest(fare_amount ~ pickup_latitude_mod+pickup_longitude_mod+ dropoff_latitude_mod+ dropoff_longitude_mod+ pickup_time+distance+direction, data=uber_dist_2015, ntree = 500, mtry=3)
